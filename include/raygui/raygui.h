 ...
 *           On gui font loading from style file, append the icons to font atlas
 * image, so, icons can be drawn along the text as a texture, instead of using
 * shapes to draw them
 *
 ...
 * guiControlExclusiveRec for exclusive modes ADDED: GuiValueBoxFloat(), with
 * floats support ADDED: GuiDropdonwBox() properties: DROPDOWN_ARROW_HIDDEN,
 * DROPDOWN_ROLL_UP ADDED: GuiListView() property: LIST_ITEMS_BORDER_WIDTH
 *                         ADDED: GuiLoadIconsFromMemory(), used by
 * GuiLoadIcons() ADDED: Macros for inputs customization, raylib decoupling
 *                         ADDED: Control result return values:
 * 1-RESULT_PRESSED, 2-RESULT_CHANGED, >2-Control_custom REMOVED: GuiSpinner()
 * from controls list, using BUTTON + VALUEBOX properties REMOVED:
 * GuiSliderPro(), functionality was redundant REMOVED: TextSplit() raylib
 * function requirement on RAYGUI_STANDALONE REDESIGNED: WARNING:
 * GuiLoadStyleFromMemory() to support icons baking into font atlas REDESIGNED:
 * GuiToggleGroup() to process rows/cols with no need for GuiTextSplit()
 *                         REDESIGNED: GuiColorPanel(), improved HSV <-> RGBA
 * convertion REDESIGNED: WARNING: TEXT_LINE_SPACING does not consider text
 * height, only lines spacing REDESIGNED: WARNING: GuiMessageBox(), added
 * parameter for btn return, unify result REDESIGNED: WARNING:
 * GuiTextInputBox(), added parameter for btn return, unify result REVIEWED:
 * GuiLoadIconsFromMemory(), fixed memory issues REVIEWED: Controls using text
 * labels to use LABEL properties REVIEWED: Replaced sprintf() by snprintf() for
 * more safety REVIEWED: GuiTabBar(), close tab with mouse middle button
 *                         REVIEWED: GuiScrollPanel(), scroll speed proportional
 * to content REVIEWED: GuiDropdownBox(), support roll up and hidden arrow
 *                         REVIEWED: GuiTextBox(), cursor position
 * initialization REVIEWED: GuiSliderPro(), control value change check REVIEWED:
 * GuiGrid(), simplified implementation REVIEWED: GuiIconText(), increase buffer
 * size and reviewed padding REVIEWED: GuiDrawText(), improved wrap mode drawing
 *                         REVIEWED: GuiScrollBar(), minor tweaks
 *                         REVIEWED: GuiProgressBar(), improved borders
 ...
 *
 *   DEPENDENCIES:
 *       raylib 6.1-dev  - Inputs reading (keyboard/mouse), shapes drawing, font
...
typedef enum {
  ...
  TEXT_ALIGNMENT =
      14, // Control text horizontal alignment inside control text bound (after
          // border and padding): 0-Left, 1-Center, 2-Right
  BASEPROP16 = 15 // Not used yet...
} GuiControlProperty;

// WARNING: Some properties have been chosen to be global with DEFAULT -
// EXTENDED properties:
...
//    - TEXT_PADDING                // Control text padding, not considering
//    border
//    - TEXT_ALIGNMENT              // Control text horizontal alignment inside
//    control text bound

// Controls EXTENDED properties (RAYGUI_MAX_PROPS_EXTENDED = 8)
// WARNING: Only 8 slots vailable for those properties per control, including
// DEFAULT
//----------------------------------------------------------------------------------
// DEFAULT control, extended properties
// NOTE: Those properties are global for all controls, they can not be setup per
// control
typedef enum {
  ...
  TEXT_ALIGNMENT_VERTICAL =
      21, // Text vertical alignment inside text bounds (after border and
          // padding): 0-Top, 1-Middle, 2-Bottom
...
// line thickness TEXT_FONT_WEIGHT              // Text font weight: 0-Normal,
// 1-Bold, 2-Italic -> Requires specific font change
...
typedef enum {
  ...
  GROUP_WIDTH_FULL // ToggleGroup bounds width considers all items: 0-Width per
                   // item, 1-Full width
} GuiToggleProperty;
...
typedef enum {
  TEXT_READONLY =
      16, // TextBox in read-only mode: 0-Text editable, 1-Text read-only
} GuiTextBoxProperty;
...
GuiLoadIcons(const char *fileName,
             bool loadIconsName); // Load raygui icons file (.rgi) into internal
                                  // icons data
RAYGUIAPI char **GuiLoadIconsFromMemory(
    const unsigned char *fileData, int dataSize,
    bool loadIconsName); // Load raygui icons file (.rgi) from memory into
                         // internal icons data
RAYGUIAPI void
...
RAYGUIAPI int
GuiListViewEx(Rectangle bounds, char **text, int count, int *scrollIndex,
              int *active, int *focus); // List View control, using text entries
                                        // list and returning focus entry
RAYGUIAPI int GuiTabBar(Rectangle bounds, const char *text, int *hscroll,
                        int *active); // Tab Bar control
RAYGUIAPI int GuiTabBarEx(Rectangle bounds, char **text, int count,
                          int *hscroll, int *active,
                          int *focus); // Tab Bar control, using text entries
                                       // list and returning focus entry
RAYGUIAPI int
...
GuiTextInputBox(Rectangle bounds, const char *title, const char *message,
                char *text, int textSize, const char *btnText, int *btnActive,
...
RAYGUIAPI int GuiColorPicker(
    ...
    Color *color); // Color Picker control, includes Color bar controls
...
RAYGUIAPI int GuiColorPickerHSV(
    Rectangle bounds, const char *text,
    Vector3 *colorHsv); // Color Picker control, using Hue-Saturation-Value
                        // color data, includes Color bar controls
RAYGUIAPI int
GuiColorPanelHSV(Rectangle bounds, const char *text,
                 Vector3 *colorHsv); // Color Panel control, using
                                     // Hue-Saturation-Value color data
//----------------------------------------------------------------------------------------------------------
...
#define RAYGUI_ICON_MAX_FONT_BACKED                                            \
  257 // Maximum number of icons to back in font atlas
#define RAYGUI_ICON_MAX_NAME_LENGTH 32 // Maximum length of icon name id
#define RAYGUI_ICON_FONT_ATLAS_PADDING                                         \
  1 // Padding between backed icons in font atlas
...
static char guiFontName[32] = {
    0}; // Gui font filename, can be loaded from .rgs (Version: >=600)
...
static unsigned int guiIconFontOffsetY =
    0; // Gui icon font atlas offset (if icons backed)
...
//----------------------------------------------------------------------------------
static int
GetLineWidth(const char *text); // Get text line width (stops at '\n' or '\0')
static Rectangle
...
static char **
GuiTextSplit(const char *text, char delimiter,
             int *count); // Split controls text into multiple strings
static Vector3
...
static int GuiFontIconBaking(
    Image *imFont, Font font,
    Rectangle *whiteRec); // Update font image atlas to append raygui icons
...
#define RAYGUI_TOGGLEGROUP_MAX_ITEM_TEXT_SIZE 256
  ...
  // Text parsing needed to consider potential row and col entries
  // (vertical/horizontal layout) when '\n' found move vertically next toggle,
  // when ';' found move horizontally
  for (int c = 0, k = 0, itemIndex = 0, row = 0, col = 0, exit = 0; exit == 0;
       c++) {
    ...
    if (itemReady) {
      ...
      bounds.x = initBoundsX +
                 col * (bounds.width + GuiGetStyle(TOGGLE, GROUP_PADDING));
      bounds.y = initBoundsY +
                 row * (bounds.height + GuiGetStyle(TOGGLE, GROUP_PADDING));
...
int GuiTabBarEx(Rectangle bounds, char **text, int count, int *hscroll,
                int *active, int *focus) {
  int result = RESULT_NONE;
  // GuiState state = guiState;

  int tabItemsWidth = GuiGetStyle(TABBAR, TAB_ITEMS_WIDTH);
  Rectangle tabBounds = {bounds.x, bounds.y, (float)tabItemsWidth,
                         bounds.height};
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (tabBounds.x < GetScreenWidth()) {
      ...
      if (GuiGetStyle(TABBAR, TAB_CLOSE_BUTTON)) {
        ...
        GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
        if (GuiButton(RAYGUI_CLITERAL(Rectangle){tabBounds.x + tabBounds.width -
                                                     14 - 5,
                                                 tabBounds.y + 5, 14, 14},
                      "x"))
          result = RESULT_TAB_CLOSE;
  ...
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height - 1,
                                 bounds.width, 1},
      0, BLANK, GetColor(GuiGetStyle(TABBAR, BORDER_COLOR_NORMAL)));
  //--------------------------------------------------------------------
...
int GuiColorPanel(Rectangle bounds, const char *text, Color *color) {
  ...
  Vector3 prevHsv =
      hsv; // NOTE: Workaround to see if GuiColorPanelHSV() modifies the hsv
...
int GuiColorBarHue(Rectangle bounds, const char *text, float *hue) {
  ...
  if (state != STATE_DISABLED) {
    ...
    // NOTE: Using DrawRectangleGradientEx(bounds, color1, color2, color2,
    // color1);
    DrawRectangleGradientEx(
        RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, bounds.width,
                                   bounds.height / 6.0f},
    ...
    DrawRectangleGradientEx(
        RAYGUI_CLITERAL(Rectangle){bounds.x,
                                   bounds.y + 1 * (bounds.height / 6.0f),
                                   bounds.width, bounds.height / 6.0f},
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
...
int GuiColorPanelHSV(Rectangle bounds, const char *text, Vector3 *colorHsv) {
  ...
  if ((prevColorHsv.x != colorHsv->x) || (prevColorHsv.y != colorHsv->y) ||
...
// Message Box control
// NOTE: Button pressed is returned through btnActive parameter, 0 for window
// close button
...
#define RAYGUI_TEXTINPUTBOX_HEIGHT 26
  ...
  if (secretViewActive != NULL) {
    ...
    if (GuiTextBox(RAYGUI_CLITERAL(Rectangle){textBoxBounds.x, textBoxBounds.y,
                   ...
                   textSize, textEditMode) == RESULT_PRESSED)
    ...
    if (GuiTextBox(textBoxBounds, text, textSize, textEditMode) ==
        RESULT_PRESSED)
      textEditMode = !textEditMode;
  }
...
void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize) {
  ...
  // 6       | 2       | short      | reserved
  // 8       | 4       | int        | Num properties (only changed ones from
  // default style)

  // Properties Data (8 bytes per property)
  // WARNING: Only properties required that differ from default (light) internal
  ...
  // ...     | 4       | int        | Font data size (0 - no font, no more
  // fields added!)
  // ...     | 32      | char       | Font filename (with extension) - VERSION:
  // >=600
  ...
  // ...     | 4       | int        | Recs data compressed size (0 - not
  // compressed, 1-compressed) - VERSION: >=400
  //
  // if (compRecsSize == 0)
  // {
  ...
  // ...     | 4       | int        | Glyphs data compressed size (0 - not
  // compressed) - VERSION: >=400
  //
  // if (compGlyphsSize == 0)
  // {
  //    NOTE: Uncompressed size can be calculated: (glyphCount*16 byte)
  ...
  if ((signature[0] == 'r') && (signature[1] == 'G') && (signature[2] == 'S') &&
    ...
    if (fontDataSize > 0) {
      ...
      // WARNING: Version 600 adds 32 bytes for the font filename (with
      // extension)
      if (version >= 600) {
        // GLOBAL: Copy font file name into guiFontName
...
void GuiLoadStyleDefault(void) {
  ...
  GuiSetStyle(DEFAULT, TEXT_LINE_SPACING,
              12); // DEFAULT, pixels between lines, from bottom of first line
                   // to top of second
...
char **GuiLoadIconsFromMemory(const unsigned char *fileData, int dataSize,
  ...
  // icon size) Size*Size pixels/32bit per unsigned int = K unsigned int per
  // icon foreach (icon)
  // {
  ...
  if ((signature[0] == 'r') && (signature[1] == 'G') && (signature[2] == 'I') &&
    ...
    if (loadIconsName) {
      ...
      guiIconsName =
          (char **)RAYGUI_CALLOC(RAYGUI_ICON_MAX_ICONS, sizeof(char *));
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color) {
  if ((guiIconFontOffsetY > 0) && (iconId < RAYGUI_ICON_MAX_FONT_BACKED)) {
    int maxIconsPerLine =
        guiFont.texture.width /
        (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING);
    int x = iconId % maxIconsPerLine;
    int y = iconId / maxIconsPerLine;

    Rectangle srcRec = {
        (float)x * (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING) +
            RAYGUI_ICON_FONT_ATLAS_PADDING,
        guiIconFontOffsetY +
            (float)y * (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING) +
            RAYGUI_ICON_FONT_ATLAS_PADDING,
        RAYGUI_ICON_SIZE, RAYGUI_ICON_SIZE};
    Rectangle dstRec = {(float)posX, (float)posY,
                        (float)pixelSize * RAYGUI_ICON_SIZE,
                        (float)pixelSize * RAYGUI_ICON_SIZE};

    DrawTexturePro(guiFont.texture, srcRec, dstRec,
                   RAYGUI_CLITERAL(Vector2){0, 0}, 0.0f, color);
  } else {
  ...
  for (int i = 0; i < lineCount; i++) {
    ...
    if (iconId >= 0) {
      ...
      GuiDrawIcon(iconId, (int)textBoundsPosition.x,
                  ...
                  guiIconScale, tint);
      textBoundsPosition.x +=
          (float)(RAYGUI_ICON_SIZE * guiIconScale + RAYGUI_ICON_TEXT_PADDING);
...
static void GuiTooltip(Rectangle controlRec) {
  if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) &&
      !guiControlExclusiveMode) {
    Vector2 textSize = MeasureTextEx(guiFont, guiTooltipPtr,
                                     (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
                                     (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
...
static int GuiFontIconBaking(Image *imFont, Font font, Rectangle *whiteRec) {
  ...
  int maxIconsPerLine =
      imFont->width / (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING);
  int reqIconLines = RAYGUI_ICON_MAX_FONT_BACKED / maxIconsPerLine +
                     RAYGUI_ICON_MAX_FONT_BACKED % maxIconsPerLine +
                     1; // One extra line
  int reqHeight =
      reqIconLines * (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING);
  ...
  if ((maxGlyphRecY + reqHeight) > imFont->height) {
    ...
    char *newImData =
        (char *)RL_CALLOC(imFont->width * newImHeight * 2, sizeof(char));
    ...
    for (int y = 0; y < 4; y++)
      for (int x = 0; x < 4; x++)
        ((unsigned short *)newImData)[(imFont->height - y - 1) * imFont->width +
                                      imFont->width - x - 1] = 0x0000;
    ...
    for (int y = 0; y < 3; y++)
      for (int x = 0; x < 3; x++)
        ((unsigned short *)newImData)[(imFont->height - y - 1) * imFont->width +
                                      imFont->width - x - 1] = 0xffff;

    *whiteRec = RAYGUI_CLITERAL(Rectangle){(float)imFont->width - 2,
                                           (float)imFont->height - 2, 1, 1};
  }

  // Calculate image offset positions to start drawing
  // NOTE: For Y, look for a multiple of icon size + 2*padding, for better
  // alignment
  int offsetX = RAYGUI_ICON_FONT_ATLAS_PADDING;
  int offsetY = ((maxGlyphRecY +
                  (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING - 1)) /
                 (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING)) *
                    (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING) +
                RAYGUI_ICON_FONT_ATLAS_PADDING;
...
#define BIT_CHECK(a, b) ((a) & (1u << (b)))
  ...
  for (int iconId = 0; iconId < RAYGUI_ICON_MAX_FONT_BACKED; iconId++) {
    ...
    if ((offsetX + (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING)) >
        imFont->width) {
    ...
    for (int i = 0, y = 0; i < RAYGUI_ICON_DATA_ELEMENTS; i++) {
      ...
      for (int k = 0; k < 32; k++) {
        pixels[(offsetY + y) * imFont->width + offsetX + k % 16] =
            (BIT_CHECK(data, k)) ? 0xffff : 0x00ff;
...
// NOTE: Re-used by GuiToggleSlider(), GuiComboBox(), GuiDropdownBox(),
// GuiListView(), GuiMessageBox(), GuiInputBox()
...
#define RAYGUI_TEXTSPLIT_MAX_TEXT_SIZE                                         \
  1024 // WARNING: Max expected size for all concat items
#endif

  static char *itemPtrs[RAYGUI_TEXTSPLIT_MAX_ITEMS] = {
      0}; // String pointers array (points to buffer data)
