 ...
 * modes ADDED: GuiValueBoxFloat(), with floats support ADDED: GuiDropdonwBox() properties:
 * DROPDOWN_ARROW_HIDDEN, DROPDOWN_ROLL_UP ADDED: GuiListView() property: LIST_ITEMS_BORDER_WIDTH
 *                         ADDED: GuiLoadIconsFromMemory(), used by GuiLoadIcons()
 *                         ADDED: Macros for inputs customization, raylib decoupling
 *                         ADDED: Control result return values: 1-RESULT_PRESSED, 2-RESULT_CHANGED,
 * >2-Control_custom REMOVED: GuiSpinner() from controls list, using BUTTON + VALUEBOX properties
 *                         REMOVED: GuiSliderPro(), functionality was redundant
 *                         REMOVED: TextSplit() raylib function requirement on RAYGUI_STANDALONE
 *                         REDESIGNED: WARNING: GuiLoadStyleFromMemory() to support icons baking
 * into font atlas REDESIGNED: GuiToggleGroup() to process rows/cols with no need for GuiTextSplit()
 *                         REDESIGNED: GuiColorPanel(), improved HSV <-> RGBA convertion
 *                         REDESIGNED: WARNING: TEXT_LINE_SPACING does not consider text height,
 * only lines spacing REDESIGNED: WARNING: GuiMessageBox(), added parameter for btn return, unify
 * result REDESIGNED: WARNING: GuiTextInputBox(), added parameter for btn return, unify result
 ...
 *
 *   DEPENDENCIES:
 *       raylib 6.1-dev  - Inputs reading (keyboard/mouse), shapes drawing, font loading and text
 * drawing
 *
 *   STANDALONE MODE:
...
typedef enum {
  ...
  TEXT_ALIGNMENT = 14,  // Control text horizontal alignment inside control text bound (after border
                        // and padding): 0-Left, 1-Center, 2-Right
...
typedef enum {
  ...
  TEXT_ALIGNMENT_VERTICAL = 21,  // Text vertical alignment inside text bounds (after border and
...
// TEXT_FONT_WEIGHT              // Text font weight: 0-Normal, 1-Bold, 2-Italic -> Requires
// specific font change
...
RAYGUIAPI void GuiLoadStyleFromMemory(const unsigned char* fileData,
...
RAYGUIAPI char** GuiLoadIconsFromMemory(
    ...
    int                  dataSize,
    bool loadIconsName);  // Load raygui icons file (.rgi) from memory into internal icons data
...
RAYGUIAPI int
GuiToggleGroup(Rectangle bounds, const char* text, int* active);            // Toggle Group control
RAYGUIAPI int
GuiToggleSlider(Rectangle bounds, const char* text, int* active);           // Toggle Slider control
RAYGUIAPI int GuiCheckBox(Rectangle   bounds,
                          const char* text,
...
RAYGUIAPI int GuiValueBox(Rectangle   bounds,
                          const char* text,
                          int*        value,
                          int         minValue,
                          int         maxValue,
                          bool editMode);  // Value Box control, updates input text with numbers
RAYGUIAPI int GuiValueBoxFloat(Rectangle   bounds,
                               const char* text,
                               char*       textValue,
                               float*      value,
                               bool        editMode);  // Value box control for float values
RAYGUIAPI int GuiTextBox(Rectangle bounds,
                         char*     text,
                         int       textSize,
...
RAYGUIAPI int
GuiListViewEx(Rectangle bounds,
              char**    text,
              int       count,
              int*      scrollIndex,
              int*      active,
              int* focus);  // List View control, using text entries list and returning focus entry
RAYGUIAPI int
GuiTabBar(Rectangle bounds, const char* text, int* hscroll, int* active);  // Tab Bar control
RAYGUIAPI int
GuiTabBarEx(Rectangle bounds,
            char**    text,
            int       count,
            int*      hscroll,
            int*      active,
            int*      focus);  // Tab Bar control, using text entries list and returning focus entry
...
RAYGUIAPI int
GuiTextInputBox(Rectangle   bounds,
                const char* title,
                const char* message,
                char*       text,
                int         textSize,
...
RAYGUIAPI int GuiColorPicker(Rectangle   bounds,
                             const char* text,
...
RAYGUIAPI int
GuiColorPickerHSV(Rectangle   bounds,
                  const char* text,
                  Vector3*    colorHsv);  // Color Picker control, using Hue-Saturation-Value color
                                          // data, includes Color bar controls
RAYGUIAPI int
GuiColorPanelHSV(Rectangle   bounds,
                 const char* text,
                 Vector3* colorHsv);  // Color Panel control, using Hue-Saturation-Value color data
//----------------------------------------------------------------------------------------------------------
...
static unsigned int guiIcons[RAYGUI_ICON_MAX_ICONS * RAYGUI_ICON_DATA_ELEMENTS] = {
    ...
    0x3ff00000, 0x201c2010, 0x21042004, 0x22842384,
    0x264426c4, 0x2c242fe4, 0x20043e74, 0x00003ffc,  // ICON_FILETYPE_FONT
    0x3ff00000, 0x201c2010, 0x20042004, 0x27742004,
    0x29742944, 0x27742944, 0x20042004, 0x00003ffc,  // ICON_FILETYPE_3D
    0x3ff00000, 0x201c2010, 0x20042004, 0x20042004,
    0x24242244, 0x24242814, 0x20042244, 0x00003ffc,  // ICON_FILETYPE_XML
    0x3ff00000, 0x201c2010, 0x20042004, 0x20042004,
    0x21042f04, 0x21042104, 0x20042f04, 0x00003ffc,  // ICON_FILETYPE_C
    0x3ff00000, 0x201c2010, 0x23842004, 0x2bf42a04,
    0x2fd42814, 0x21c42054, 0x20042004, 0x00003ffc,  // ICON_FILETYPE_PYTHON
    0x3ff00000, 0x201c2010, 0x20042004, 0x20042004,
    0x22842ee4, 0x28a42e84, 0x20042e64, 0x00003ffc,  // ICON_FILETYPE_JS
    0x3ff00000, 0x241c2010, 0x2a8c3104, 0x28242454,
    0x2ed42004, 0x2a542a54, 0x20042ed4, 0x00003ffc,  // ICON_FILETYPE_ICON
    0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x00000000, 0x00000000, 0x00000000,  // ICON_257
};
...
static char*
LoadFileText(const char* fileName);      // -- GuiLoadStyle(), required to load charset data
...
static char**  GuiTextSplit(const char* text,
                            char        delimiter,
...
static int  GuiScrollBar(Rectangle bounds,
                         int       value,
                         int       minValue,
                         int       maxValue);  // Scroll bar control, used by GuiScrollPanel()
static int
GuiFontIconBaking(Image*     imFont,
                  Font       font,
...
// NOTE: This define is also used by GuiMessageBox() and GuiTextInputBox()
  ...
  Rectangle   windowPanel    = {bounds.x,
                                bounds.y + (float)statusBarHeight - (float)statusBorderWidth,
                                bounds.width,
...
int GuiGroupBox(Rectangle bounds, const char* text) {
  ...
  GuiDrawRectangle(
      ...
      GetColor(
          GuiGetStyle(DEFAULT,
                      (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
  GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle){bounds.x,
                   ...
                   GetColor(GuiGetStyle(DEFAULT,
                                        (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED
                                                                  : (int)LINE_COLOR)));
  ...
  GuiLine(RAYGUI_CLITERAL(Rectangle){bounds.x,
  ...
  GuiDrawRectangle(bounds,
                   ...
                   GetColor(GuiGetStyle(DEFAULT,
                                        (state == STATE_DISABLED) ? (int)BORDER_COLOR_DISABLED
                                                                  : (int)LINE_COLOR)),
                   GetColor(GuiGetStyle(DEFAULT,
                                        (state == STATE_DISABLED) ? (int)BASE_COLOR_DISABLED
...
int GuiDropdownBox(Rectangle bounds, const char* text, int* active, bool editMode) {
  ...
  if (!GuiGetStyle(DROPDOWNBOX, DROPDOWN_ARROW_HIDDEN)) {
    ...
    GuiDrawText("v",
                ...
                GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state * 3))));
#else
    GuiDrawText(direction ? GuiIconText(ICON_ARROW_UP_FILL, NULL)
...
int GuiDummyRec(Rectangle bounds, const char* text) {
  ...
  GuiDrawText(
      ...
      GetColor(GuiGetStyle(BUTTON,
                           (state != STATE_DISABLED) ? TEXT_COLOR_NORMAL : TEXT_COLOR_DISABLED)));
  //------------------------------------------------------------------
    ...
    if (state == STATE_DISABLED) {
      ...
      if (((startIndex + i) == itemSelected) && (active != NULL)) {
        ...
        GuiDrawText(text[startIndex + i],
                  ...
                  itemFocused))  // && (focus != NULL)) // NOTE: Items focused, despite not returned
      {
        // Draw item focused
        GuiDrawRectangle(itemBounds,
                         GuiGetStyle(LISTVIEW, LIST_ITEMS_BORDER_WIDTH),
...
int GuiTabBarEx(Rectangle bounds, char** text, int count, int* hscroll, int* active, int* focus) {
  ...
  // Draw control
  //--------------------------------------------------------------------
  // if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode) // TODO: Support
  // disabled
  ...
  GuiDrawRectangle(
      RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + bounds.height - 1, bounds.width, 1},
      0,
      BLANK,
...
int GuiColorBarHue(Rectangle bounds, const char* text, float* hue) {
  ...
  if (state != STATE_DISABLED) {
    ...
    DrawRectangleGradientEx(
        RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, bounds.width, bounds.height / 6.0f},
        Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
        Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
        Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha));
    DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x,
...
int GuiTextInputBox(Rectangle   bounds,
  ...
  if (secretViewActive != NULL) {
    static char stars[] = "****************";
    if (GuiTextBox(RAYGUI_CLITERAL(Rectangle){textBoxBounds.x,
                                              textBoxBounds.y,
...
void GuiLoadStyleFromMemory(const unsigned char* fileData, int dataSize) {
  ...
  // ...     | 4       | int        | Recs data compressed size (0 - not compressed, 1-compressed) -
  // VERSION: >=400
  //
  // if (compRecsSize == 0)
  // {
  ...
  // ...     | 4       | int        | Glyphs data compressed size (0 - not compressed) - VERSION:
  // >=400
  //
  // if (compGlyphsSize == 0)
  // {
  ...
  if ((signature[0] == 'r') && (signature[1] == 'G') && (signature[2] == 'S') &&
    ...
    if (fontDataSize > 0) {
      ...
      if ((recsDataCompressedSize > 0) && (recsDataCompressedSize != recsDataSize)) {
        ...
        font.recs              = (Rectangle*)DecompressData(recsDataCompressed,
...
void GuiLoadStyleDefault(void) {
  ...
  GuiSetStyle(DEFAULT,
              TEXT_LINE_SPACING,
              12);  // DEFAULT, pixels between lines, from bottom of first line to top of second
  GuiSetStyle(DEFAULT,
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color) {
  if ((guiIconFontOffsetY > 0) && (iconId < RAYGUI_ICON_MAX_FONT_BACKED)) {
    int maxIconsPerLine =
        guiFont.texture.width / (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING);
    int x = iconId % maxIconsPerLine;
    int y = iconId / maxIconsPerLine;

    Rectangle srcRec = {(float)x * (RAYGUI_ICON_SIZE + 2 * RAYGUI_ICON_FONT_ATLAS_PADDING) +
...
#define TEXT_VALIGN_PIXEL_OFFSET(h) ((int)h % 2)  // Vertical alignment for pixel perfect
  ...
  for (int i = 0; i < lineCount; i++) {
    ...
    for (int c = 0, codepointSize = 0; c < lineSize; c += codepointSize) {
      ...
      // NOTE: Normally, exiting the decoding sequence as soon as a bad byte is found (and return
      // 0x3f) but all of the bad bytes need to be drawn using the '?' symbol, moving one byte
      if (codepoint == 0x3f)
...
static void GuiTooltip(Rectangle controlRec) {
  if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiControlExclusiveMode) {
    Vector2 textSize = MeasureTextEx(guiFont,
                                     guiTooltipPtr,
                                     (float)GuiGetStyle(DEFAULT, TEXT_SIZE),
                                     (float)GuiGetStyle(DEFAULT, TEXT_SPACING));
  ...
  if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE)) {
    ...
    GuiDrawText(
        ...
        RAYGUI_CLITERAL(Rectangle){arrowUpLeft.x,
                                   ...
                                   isVertical ? bounds.width : bounds.height},
        TEXT_ALIGN_CENTER,
        GetColor(GuiGetStyle(SCROLLBAR,
...
static int GuiFontIconBaking(Image* imFont, Font font, Rectangle* whiteRec) {
  ...
  int reqIconLines    = RAYGUI_ICON_MAX_FONT_BACKED / maxIconsPerLine +
                        RAYGUI_ICON_MAX_FONT_BACKED % maxIconsPerLine + 1;  // One extra line
  ...
  if ((maxGlyphRecY + reqHeight) > imFont->height) {
    ...
    for (int y = 0; y < 4; y++)
      for (int x = 0; x < 4; x++)
        ((unsigned short*)
             newImData)[(imFont->height - y - 1) * imFont->width + imFont->width - x - 1] = 0x0000;
    ...
    for (int y = 0; y < 3; y++)
      for (int x = 0; x < 3; x++)
        ((unsigned short*)
             newImData)[(imFont->height - y - 1) * imFont->width + imFont->width - x - 1] = 0xffff;

    *whiteRec =
        RAYGUI_CLITERAL(Rectangle){(float)imFont->width - 2, (float)imFont->height - 2, 1, 1};
  }
...
// NOTE: Re-used by GuiToggleSlider(), GuiComboBox(), GuiDropdownBox(), GuiListView(),
// GuiMessageBox(), GuiInputBox()
